...
export const App = () => {
    ...
    useEffect(() => {
        if (userId) {
            ...
            const handleSessionTerminated = (payload: { reason: string }) => {
                alert(
                    payload.reason === "another_session_detected"
    ...
    return (
        <AppProviders>
            {isRefreshing && <div className="banner banner-info">Refreshing session...</div>}
            {isReconnecting && <div className="banner banner-warning">Reconnecting...</div>}
            <AppRoutes />
        </AppProviders>
    );
};
